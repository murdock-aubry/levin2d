...
subroutine chebyshev_interp5(n, xscheb, vals1, vals2, xs, valsout1, valsout2)
    implicit double precision (a-h, o-z)
    double precision                        :: xs(:), xscheb(:)
    double complex                          :: vals1(:), vals2(:)
    double complex                          :: valsout1(:), valsout2(:)
    double precision, allocatable           :: interpmat(:,:)
    ...
    do i = 1, mout
        call chebyshev_interp(n,xscheb,vals1,xs(i),valsout1(i))
        call chebyshev_interp(n,xscheb,vals2,xs(i),valsout2(i))
    end do

    


    ! allocate(valsout1(mout), valsout2(mout))

    ! call chebyshev_interpmatrix(n, mout, xs, interpmat)

    ! print *, size(interpmat, 1), size(interpmat, 2)
    ! call prind("", interpmat)

    ! valsout1 = matmul(transpose(interpmat), vals1)
    ! valsout2 = matmul(transpose(interpmat), vals2)
